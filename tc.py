...
def getKeyList(entity_type, entity_id, isTelemetry=True):
    ...
    JWT_Token = getToken()
    if isTelemetry:
        url = 'http://localhost:8080/api/plugins/telemetry/%s/%s/keys/timeseries' %(entity_type,entity_id)
    else:
        url = 'http://localhost:8080/api/plugins/telemetry/%s/%s/keys/attributes' %(entity_type,entity_id)
